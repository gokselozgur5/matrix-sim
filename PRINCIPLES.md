...
**Dev12 — Units ship whole (D-039, D-063).** One coherent `build-unit` issue = one small PR that closes it with keywords; atomic commit messages carry independently revertible findings, fixes and evidence; light locks run per PR (compile · `--selftest` · digest leash, `--bench` where speed is touched), with the full adversarial pass at phase boundaries. Throughput may break ties among comparably valuable legitimate ready units after value and dependencies; it never slices one natural unit into contribution-shaped pieces. No hundred-commit PRs, no empty issues — detail is the deliverable, volume is its shadow.
...
**Dev14 — Quality admits; value leads; throughput breaks ties (D-063 amends D-060).** Every artifact first names the human capability, world behavior, proof or demonstrated process risk that makes it independently necessary, keeps its natural boundary, and passes its evidence. Human/world value chooses the lane and dependency value orders it. Only among comparably valuable ready units may backlog reduction and the commit–issue–PR–review shape break a tie. `tools/balance.sh` is advisory: a thin leg can choose between such peers but cannot create one, outrank value, split one, or weaken a lock. Reviews are earned by adversarial passes that could have found something — never by ceremony, and never by a second identity approving the first's work.
