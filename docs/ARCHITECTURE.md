...
4. **No sixth document.** No `docs/CENSUS.md`, no `journal/`, no per-sweep file. The fence is checked with the entry, and since #798 it is also checked by `DocFigures` on every push rather than only by a contributor who remembers to run it:

<!-- figure: git ls-files '*.md' | grep -vE '^docs/adr/|^docs/spec/|^tools/|^probes/|^\.github/|^ci/' | wc -l == 6 -->

   `git ls-files '*.md' | grep -vE '^docs/adr/|^docs/spec/|^tools/|^probes/|^\.github/|^ci/' | wc -l` must print **6**. The excluded prefixes are the **sanctioned kinds**, and the pattern names them because a kind sanctioned by a decision and not taught to the fence reads as canon sprawl. `docs/spec/` was sanctioned by [D-058](adr/D-058-spec-shelf.md), whose whole definition is that a spec is *not* canon — "canon is prose about the project, specs are portable descriptions of its rules" — and `ci/` holds the fixtures CI is identical to. Neither is a sixth document. What the fence counts is the five canon files plus the door, and that number is unchanged since D-028.
