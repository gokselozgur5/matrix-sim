...
| `AllocMeter` | What does the hot path allocate, really? | retired D-027's never-measured "allocation-free" row, then guarded the row that replaced it: steady bytes/tick and GC collections judged against the record's own 32 KB and 5, `VERDICT ALLOC_IN_BUDGET` (#906); `--selfcheck` runs the comparison's four cases and the settle band's two with no universe, because at these figures neither branch is otherwise reachable. Then it caught itself: the 1,963-8,537 B/tick it used to report was one cold sample of a window C2 was still compiling into, and twenty-four repeats on fresh simulations converge to **365-367 B/tick at seed 42, 423-425 at seed 7, 0 GCs per arc** — the bound has ~90x of room, not 4x (#817). The old headline still prints, as `steady_max`. **A scaled line** (`AllocMeter <seed> <scale>`) is that same instrument over a bigger city and is never judged — no record bounds bytes at any scale, so it closes on `VERDICT ALLOC_UNJUDGED` — but since #979 it is settled before it is printed: the ramp to the floor takes eight to ten repeats at scale 1, 2, 4 and 11 alike, because C2 counts invocations and not ticks, so #977's divide-by-scale published **763 B/tick against its own floor of 475 at x2, and 1,110 against a floor of 720-757 at x11** where three cold samples agreed with each other to 0.09%. Twenty-four repeats at every scale, costing 3.2→5.5 s at x2 and 18→42 s at x11, land **475 at x2 (floor 468-475), 403 at seed 7 x2 (floor 403), 725-757 at x11 (floor 720-757)**; and the settle check rides every line, canonical included — a median more than 5% above its own `steady_min` appends `steady_settled=no steady_over_floor_pct=N`, which twelve consecutive runs at either scale never printed and the one seed-42 run in twenty that lands on the JIT tail (415 against a floor of 370) does, so a ramp median can no longer be quoted as a measurement by accident |
