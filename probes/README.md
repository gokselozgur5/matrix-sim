...
| `TwoWorlds` | Are two universes in one JVM actually two universes? | #1147's confirmation, and its SHAPE is the argument: "it did not throw" would have passed on main for seed 42, which produced a different chain at link 1 with no exception at all. Each seed runs alone, then all of them run concurrently, and the chains must be equal LINK FOR LINK (`WORLDS_INDEPENDENT`, four seeds, 2,000 ticks). The defect it guards was two singletons holding their neighbour scratch buffer as an instance field — one list per process wearing the syntax of one list per object |
