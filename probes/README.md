...
| `SheetFence` | Is the sheets-cached-nowhere law still true, and is there still exactly one door? | three sentences of #350's law, given a reader for the first time (#656): no wing holds a `Sheet` field, `matrix.character` imports nothing of the domain but `core`, and `SheetDoor` itself has no state to cache into. All three were already true the day this landed — `stored=0 foreign_imports=0 door_fields=0` — which is the exact shape this tree has now caught five times elsewhere (#1203, #1212, #1210, #1233, #1243): a law that is true by luck reads identically to a law that is enforced, until the first commit that breaks it. It is the STATIC half of the pair; `SheetDump`'s `cached=` is the dynamic half, walking the live heap from the composition root, and neither subsumes the other — a sheet stashed in a `static` map that no reachable field points at is invisible to the walk and plain to the grep, while a sheet parked at runtime in an `Object` field is plain to the walk and invisible to the grep. Comments are stripped before every count, because `SheetDoor`'s javadoc says `private final Sheet` out loud while explaining why no such field may exist — the self-matching shape `advice.sh` was bitten by four separate times. `--wings` and `--layer` point it at a scratch tree, which is how all three counters were seen to go red before the row was added |
